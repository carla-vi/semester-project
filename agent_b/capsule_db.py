...
def bootstrap_capsule():
    """Initial population from Agent C if DB empty"""
    ...
    try:
        ...
        raw_sock = socket.create_connection(("agent_c", 8443), timeout=10)
        tls_sock = context.wrap_socket(raw_sock, server_hostname="agent_c")
        tls_sock.send(b"sync_capsule")
        capsule = json.loads(tls_sock.recv(8192).decode())
        tls_sock.close()
        write_capsule(capsule, author="bootstrap@C")
        print("[DEBUG] Bootstrapped capsule from Agent C")
    except Exception as e:
        print("[WARN] Bootstrap from C failed:", e)


def propagate_capsule(capsule):
    """Send capsule to peers (B and C)"""
    peers = [("agent_b", 8500), ("agent_c", 8500)]
